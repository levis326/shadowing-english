...
4. 生成成功后，字幕会保存在本机并用于播放。进入“设置 → 管理 AI 字幕”，可以查看、按单词修改文本、导出、重新生成或删除字幕。如果这次只生成了外文字幕，条目上会直接写出原因（例如本地翻译模型还没下载、翻译服务返回失败），照着提示处理后重新生成即可；已生成的字幕不会因为设置变化而消失。

生成字幕会调用你选择的第三方服务，可能产生少量费用。API Key 请只填写在应用设置中，不要提交到仓库、截图或公开日志里。

### 字幕与生词

- 字幕面板里**单击单词**即可查词：应用会结合整句上下文判断这里是不是固定词组
  （`consists of` → 查 `consist of`，`as well as` 里点任意一个词都命中），是词组就
  弹词组卡片，否则弹单词卡片；词组和单词都会**还原成初始形态**再查询与显示
  （consists → consist、studies → study、children → child、went → go）。
- 没有配置在线翻译时，查词会自动使用内置离线词典或本地翻译模型，不再要求先填 API Key；
  词组若没有本地释义，会把各组成词的词典释义列出来（离线可用）。
- **播放器里双击某条字幕**（或在字幕行的“更多操作”里选“编辑这句字幕”）可以直接改这一句的
  外文/中文，保存后立即生效并写回 AI 字幕缓存（同时更新随视频保存的 `.srt`）；
  “设置 → 管理 AI 字幕”里的“编辑字幕”对**词级 AI 字幕和 `.srt` 字幕**都能用。
- 查词卡片里的「加入短语库」收藏的是**查到的词/词组（初始形态）+ 释义**，
  原句会自动记到这条短语的“学习笔记”里（想在短语库收藏整句，用字幕行右侧菜单的
  “收藏当前句型”）。
- **中文字形**：应用只打包拉丁字体的 Nunito，汉字回退顺序里显式把**简体中文字体**
  （Windows 微软雅黑 / macOS 苹方 / Linux、Android Noto Sans CJK SC）排在最前，
  避免英文界面下回退到日文/繁体中文字体导致「门、骨、直」等字形不合简体规范。
- **文字编码**：你自己带来的字幕/文本文件按内容自动识别编码 —— UTF-8（含 BOM）、
  UTF-16（记事本的“Unicode”）、GBK/GB18030（中文 Windows 的 ANSI，含四字节生僻字），
  不会再出现“字幕文件读取失败”或中文乱码；视频里内嵌字幕的提取结果同样先解码再解析。
  应用导出的 `.srt` 一律写成**带 BOM 的 UTF-8**，中文 Windows 上的外部播放器与记事本
  也能正确显示（应用自己读取时会忽略 BOM）。
- **中文排版**：AI 翻译得到的中文会自动整理成简体中文排版标准 —— 半角 `, . ? ! : ;`
  转成全角 `，。？！：；`、去掉汉字之间的多余空格；`1,000`、`3.5`、`vi.` 这类数字与
  英文缩写不受影响；你自己导入或手动编辑的中文不会被改写。
- **词根词缀（完全离线）**：查词卡片和生词本详情页会显示构词拆解
  （如 `inspection → in-（不/向内）+ spect（看）+ -tion（名词后缀）`）以及**同根词**
  （`spect → inspect / respect / aspect …`，点一下就能查看），全部用内置的词根/前缀/后缀表
  （约 380 条）和离线词典完成，不需要联网或下载模型；拆不出来的普通单词不显示该区块。
- “设置 → 管理 AI 字幕”里**每个视频只显示一条**：有词级 AI 字幕时，它的 `.srt` 副本
  并入同一条（编辑/删除会一起同步）；只有 `.srt`（例如导入课程时带的字幕）的视频才单独
  显示为“字幕文件（.srt）”。删除时会一并删除随视频保存的字幕
  副本并解绑剧集，因此打开视频不会再看到刚删掉的字幕；`.srt` 条目还可以用
  “重新生成”直接转成可逐词跟读的词级字幕。

### 已经有正确字幕文本时（不跑 AI 语音识别）

有些视频画面里已经烧录了字幕，或者你手上有这份视频完全正确的字幕文本，这时不需要让 AI 去“听写”：

1. 播放该视频，在右侧字幕面板点击“用字幕文本文件生成（不识别文字）”，选择你的字幕文件（`.srt` / `.vtt` / 纯文本 `.txt`，也可以是本应用导出的词级 JSON）。
2. 文字一律以你的文件为准：纯文本会按句读标点（`. , ! ? ; : 。 ， ！ ？ ； ：`）拆成一句一条字幕，和 AI 生成字幕的规则一致。
3. 文件没有时间轴时，应用只会用**本地的** Whisper 对齐时间轴（不上传音频、不产生费用、不联网识别文字），识别出的文字随后会被你的文本替换；文件自带时间轴（`.srt` / `.vtt`）时连音频都不用碰。
4. 之后照常按“生成双语字幕”和“翻译”设置补齐中文，并保存为可逐词跟读的词级字幕，同样出现在“设置 → 管理 AI 字幕”里。用这种方式生成的字幕，重新生成时会复用保存下来的字幕文本与时间轴，不会再次识别。

用本地方式对齐时间轴需要先在“设置 → 本地模型（在线下载）”下载 Whisper 模型；没有下载时应用会直接提示。
...
为了让安装包保持精简，语音识别、中文翻译、发音评测的模型都不再打包进安装包，而是首次使用时在「设置 → 本地模型（在线下载）」中按需下载。模型默认从 Hugging Face 镜像 `hf-mirror.com` 下载（官方站点作为回退），保存到程序数据目录的 `models/` 文件夹中，可随程序文件夹一起移动；每个文件下载后都会校验 SHA-256。

| 类别 | 模型 | 体积 | 说明 |
| --- | --- | --- | --- |
| 语音识别 | **Whisper small（多语言）★推荐** | 约 465 MB | 速度与准确率平衡最好，适合大多数电脑 |
| 语音识别 | Whisper base（轻量） | 约 141 MB | 体积小、速度快，准确率一般 |
| 语音识别 | Whisper tiny（极速） | 约 74 MB | 最小最快，准确率最低 |
| 语音识别 | Whisper medium（更准） | 约 1.4 GB | 更准但更慢，建议 8 核以上 CPU |
| 语音识别 | Whisper large-v3 turbo（高精度） | 约 1.5 GB | 接近 large 的准确率、速度更快 |
| 语音识别 | Whisper large-v3（最准） | 约 2.9 GB | 准确率最高，体积与耗时最大 |
| 中文翻译 | **NLLB-200 1.3B ★推荐** | 约 1.3 GB | 译文明显优于 600M，长句更自然 |
| 中文翻译 | NLLB-200 600M（更快） | 约 595 MB | 体积小、翻译快，个别生僻词会漏译 |
| 发音评测 | **wav2vec2 large 960h ★推荐** | 约 1.2 GB | 逐词/音节打分更准 |
| 发音评测 | wav2vec2 base 960h（轻量） | 约 360 MB | 体积小、启动快，精度略低 |

★ 为各类别推荐模型（界面中带「推荐」标记）。发音评测模型是 torchaudio 格式检查点，官方只托管在 PyTorch CDN，因此该类别从 `download.pytorch.org` 下载（界面中已标注下载源）。已下载的模型可以随时在界面中删除，也可以手动指定使用哪一个。
...
- 桌面端所有数据都保存在程序所在目录的 `data` 文件夹里（生词本、学习记录、设置、缓存、备份、已下载模型），随程序文件夹一起移动；**不会写入、也不会读取 Windows 用户目录**（`%APPDATA%`、文档目录）中的任何数据，因此新解压的程序始终从初始状态开始。界面语言同样保存在 `data/prefs.hive`；字体（Nunito）直接打包在程序内，不会在用户目录缓存字体文件。
- 项目不会提供或托管你的课程资源。
